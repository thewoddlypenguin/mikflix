...
Personal physical-media collection browser ("Mikflix"). Phase: full real-data app (632 titles) with TMDb imagery + local data pipeline + admin image overrides.
...
- Run: `start.bat` / `start.sh` (uses $APP_PORT, dev server ~3035) · Build: `npm run build` = `generate && enrich && tsc -b && vite build` · Data: `npm run generate` (CSV→titles.json), `npm run enrich` (TMDb), `npm run admin:api` (standalone image-override API), `db:import`/`db:fresh`/`db:export` (SQLite pipeline, superseded by CSV+enrich flow for frontend)
...
- Adapter maps: poster_path→posterUrl (w500), backdrop_path→backdropUrl (w1280), manual_image_url→manualImageUrl with PRIORITY over posterUrl, art_seed/art_hue/art_motif→procedural fallback art, movie→film, storage discgear→shelf. Enrichment fields (overview, genres, vote_average) exist in bundle but are NOT yet mapped — adapter still emits genres:[], rating:'NR', synopsis:'No synopsis available.'
...
- `scripts/admin-images.mjs`: zero-dep node:http API (port 4545 standalone, or /api/* via Vite dev middleware in vite.config.ts). Endpoints: GET /api/titles · POST /api/manual-image {titleId,url} · POST /api/manual-image/upload (multipart → public/images/, 8MB, jpg/png/webp/avif/gif) · DELETE /api/manual-image?titleId=
- Writes BOTH src/data/generated/titles.json AND public/titles.json; bumps generated_at. NOTE: `npm run generate` regenerates from CSV and DROPS manual_image_url (CSV has no such column) — enrich re-adds TMDb fields but NOT manual overrides. After regenerating, overrides must be re-set, or generate-titles.mjs needs to merge existing manual_image_url by id (TODO if overrides become frequent).
- Admin screen "Manual Images" panel: search, URL input, file upload, live Poster preview, remove button.

## UI Notes
- `Poster` component: renders TMDb/manual `<img>` when posterUrl given (onError → falls back to procedural canvas; effect deps include showImage so fallback redraws). `wide` prop for 16:9 hero backdrops. Canvas colors MUST use hsla helpers.
...
- `.btn`/`.btn--primary`/`.btn--ghost` styles live in home.css (Home-scoped); admin.css re-declares them scoped to `.admin` plus `.btn--danger`.
