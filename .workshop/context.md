...
- `scripts/enrich-tmdb.mjs`: TMDb search+details, cache at `scripts/tmdb-cache.json` (committed). Works OFFLINE from cache when no .env. NEVER log credential values. **Cache v3** (CACHE_VERSION=3): trailer candidates verified via YouTube oEmbed (HTTP 200 = embeddable; preference official Trailer > Trailer > Teaser; all-rejected → trailer_url null; transient oEmbed failure falls back to top candidate). v3 also adopts fresh `details:` entries through stale `search:` entries (no re-search) and short-circuits refetch when `details:` entries are fresh after generate wipes tmdb_id — version bumps cost zero API calls once warm. v2 (append_to_response=videos,credits,release_dates(mov)/content_ratings(tv) → content_rating (US cert), trailer_url (embed URL), cast (top 10, w185 profileUrl), director (movies), creators (tv); stale entries refetch once when creds exist, serve stale offline). 42 appendix/special-features titles have no TMDb match (permanent, benign).
