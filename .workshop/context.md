...
- `scripts/enrich-tmdb.mjs`: TMDb search+details, cache at `scripts/tmdb-cache.json` (committed). Works OFFLINE from cache when no .env. NEVER log credential values. **Cache v2** (CACHE_VERSION=2): append_to_response=videos,credits,release_dates(mov)/content_ratings(tv) → extracts content_rating (US cert), trailer_url (official YouTube Trailer→embed URL), cast (top 10, w185 profileUrl), director (movies), creators (tv). Stale v1 entries refetch once when creds exist, serve stale offline. 42 appendix/special-features titles have no TMDb match (permanent, benign).
